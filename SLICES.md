...
- 12 Reusable skills for the same Wisp — in progress (Proposed 12-plan-083 after 12-draft-082). Chosen demo `wisp-local-time-briefing` (Local time briefing) then 09 `wisp_tell_time`. Meeting-prep is an unavailable row. Depends on accepted 04, 06, unaccepted 07, parked 09 as the subsequent tool, and parked 10–11 overlay/inventory retest. Not Shipped.
