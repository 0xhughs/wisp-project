...
## Release evidence
Finalization pending.01–06 independentlyaccepted/archived. loop-plan-061 APPROVE_PLAN. 07, 08, 09, 10, 11, 12, 13, 14, 15, 16, 17 and 19 parked implemented-with-verification-pending; NOT Shipped; no slices/07-*.md through slices/19-*.md. 16-review-097 IMPLEMENTATION_READY (Linux 241/241; Mac raster/NSEvent/GUI backlog). 17-review-093 IMPLEMENTATION_READY (Linux 199/199; Win32/MSVC/GUI backlog). 15-review-089 IMPLEMENTATION_READY (Linux 185/185; Mac/runtime backlog). 12-review-085 IMPLEMENTATION_READY (Linux 151/151; Mac/runtime backlog). 11-review-077 IMPLEMENTATION_READY (Linux 130/130; Mac/runtime backlog). 13-review-081 IMPLEMENTATION_READY (Linux 136/136; Mac/runtime backlog). 09-review-073 IMPLEMENTATION_READY (Linux 109/109; Mac/runtime backlog). 08-review-069 IMPLEMENTATION_READY (Linux 68/68; Mac/runtime backlog). 10-review-065 IMPLEMENTATION_READY (Linux 49/49; Mac/runtime backlog). Historical 07-review-060 HUMAN_REQUIRED unchanged. 16-review-097 IMPLEMENTATION_READY parked 16 verification-pending; NOT Shipped; no slices/16-*.md. 19-review-101 IMPLEMENTATION_READY parked 19 verification-pending; NOT Shipped; no slices/19-*.md. 14-review-105 IMPLEMENTATION_READY parked 14 verification-pending; NOT Shipped; no slices/14-*.md. Next Now none (no eligible unstarted implementation; 18/20/21 product-decision blocked). 14 candidate f3e0fbabedbcd80d2ccc0dd48142d90e80ce40e5721ebbcd39348cc17a65e76a (301 files); Linux 248/248 independently reproduced. User-authorized source checkpoint ce014ffa20caab3e7c3d90ee9defd77faeb6740a published to feat/macos-release-loop. WIPpublication doesnotmean07or08or09or10or11or12or13or14or15or16or17or19accepted or releasecomplete. Existing05cloud6/6historical; extraagentallowanceunused.
...
- 14 macOS integrated readiness — implemented with verification pending. 14-review-105 IMPLEMENTATION_READY (not APPROVE_IMPLEMENTATION; counters 0/0). Linux 248/248 supplemental; NativeChecks/Mac Diagnostics GUI/launch-Quit/network/07–13 retest open. Unaccepted copy `evidence/14-contract-wip.md`. Closed readiness pack; remainder 14.x-macos-e2e. Depends on accepted 01–06 and parked 07–13. Not Shipped. Project not Complete.
...
Recommended eligible order after 07 park: 10 → 08 → 09 → 11 → 13 → 12 → 15 → 17 → 16 → 19; skip 18/20/21; 14 last among 01–14. 14 parked. No eligible unstarted implementation remains. 18/20/21 stay product-decision blocked. Do not declare Complete.
...
89. Slice 14 / Done when 1 — declared-Mac Settings → Diagnostics structured pane; keyboard Refresh/Copy; omit-list holds with a test sentinel; no composer. Requires declared Mac and participant. Automated: Linux-supplemental snapshot/redaction (`sh tools/linux-js-tests.sh`, 248/248 at candidate `f3e0fbabedbcd80d2ccc0dd48142d90e80ce40e5721ebbcd39348cc17a65e76a`; product `a0088b4939613effc8de2ec134b69a7b3ee39e5e`). Procedure: isolated app; Diagnostics; Refresh; Copy. Expected: labeled nonsecret facts; Refresh does not wake voice or prompt; Copy matches on-screen redacted text. Invalidated by DiagnosticsView / CompanionController Diagnostics composition / ManagementState Diagnostics copy / SettingsWindowController.

90. Slice 14 / Done when 2 — live local-only session: operator-recorded absence of contact with `harness-telemetry.deepseeksvc.com`; Diagnostics still reports telemetry disabled. Source inspection is not this pass. Requires declared Mac. Invalidated by overlay, `environment()`, `body-bridge` spawn env, `DSH_TELEMETRY_*`.

91. Slice 14 / Done when 3 — launch missing-home: Settings/Quit usable; Diagnostics homeConfigured false without path; menu Quit no orphan; relaunch one status item; forced-bridge-stop distinguished from clean Quit. Requires declared Mac. Invalidated by CompanionController terminate/lifecycle, EngineBridge, run-macos.sh.

92. Slice 14 / Done when 4 — follow documented local install/run on declared Mac (ad-hoc codesign only); record OS/hardware as observation not SLA; **07–13 retest** as in the 14 contract Done when 4. Do not spend DeepSeek 0/1. Do not treat 07–13 as accepted. Invalidated by launch scripts, overlay, CompanionController, and per-slice sources already listed in backlog items 1–45.

93. Slice 14 / Tests 4 native — compile NativeChecks including DiagnosticsSnapshot / DiagnosticsView / ManagementState omit-list. Requires macOS SDK. Linux: missing-platform. Procedure: `desktop/scripts/test-native.sh` and `desktop/scripts/build-macos.sh`. Invalidated by those Swift sources or test-native.sh list.

94. Slice 14.x-macos-e2e remainder — live spoken 07–13, live OTLP silence, ordinary desktop usability, and independent 01–14 APPROVE_IMPLEMENTATION remain open. Invalidated if a later contract claims those Linux-passed or writes `slices/07-*.md` through `slices/14-*.md` without independent acceptance.

95. Slice 14 protocol non-touch — overlay, `product-sdk.ts`, `permission-protocol.mjs`, `ax-actions.mjs`, `visual-actions.mjs`, `spike/**`, and `desktop/windows/**` remain unchanged vs 14 Building dispatch `7b12c4ba784d009e4412c176c9be2163bb139cae` except assertion-only overlay/env reads. Invalidated by edits to those paths.
