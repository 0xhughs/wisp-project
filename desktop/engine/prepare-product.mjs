...
export const productFiles=['product-sdk.ts','approval-policy.ts','permission-fixtures.ts','local-permission-plugin.ts','compatible-plugin.ts','compatible-verification.ts','child-verification.ts','queue-verification.ts','product.patch.yml'];
...
export function prepareProduct(runtime,source=dirname(fileURLToPath(import.meta.url))) {
 ...
 function directory(path){try{mkdirSync(path,{mode:0o700})}catch(e){if(e.code!=='EEXIST')throw e}const s=lstatSync(path);if(!s.isDirectory()||s.isSymbolicLink()||s.uid!==process.getuid())throw Error('PRODUCT_DIRECTORY');}
 ...
 for(const name of productFiles) {
  ...
  const temp=join(destination,'.'+randomUUID());writeFileSync(temp,bytes,{mode:0o600,flag:'wx'});renameSync(temp,target);
 }
