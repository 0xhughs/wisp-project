...
export function validateRequest(value) {
 ...
 if(!fields(value,keys)||value.version!==1||!['generation','requestId','sessionId','callId','rootCallId','companionId'].every(k=>id(value[k]))||value.rootCallId!==value.callId||!Number.isSafeInteger(value.turn)||value.turn<1||!/^[a-f0-9]{64}$/.test(value.actionDigest)||!['wisp_permission_check','wisp_plugin_check'].includes(value.toolName)||value.source!==(value.toolName==='wisp_permission_check'?'wisp-direct':'wisp-local-plugin')||value.revision!=='1'||value.operation!=='append-test-record'||!fields(value.arguments,['label'])||!/^[A-Za-z0-9_-]{1,40}$/.test(value.arguments.label)||!text(value.destination,2048)||!Array.isArray(value.fields)||value.fields.length>12||!value.fields.every(f=>fields(f,['label','value'])&&text(f.label,80)&&text(f.value,2048))||Buffer.byteLength(JSON.stringify(value))>12000)throw Error('PERMISSION_REQUEST');
 return value;
}
