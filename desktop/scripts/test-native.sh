...
swiftc "$src/VoiceShortcut.swift" "$src/VoiceState.swift" "$src/VoiceStore.swift" "$src/VoiceController.swift" "$src/VoiceLifecycle.swift" "$src/SpeechProviders.swift" "$src/PermissionState.swift" "$src/ModelsFocus.swift" "$src/ReasoningStore.swift" "$src/ReasoningCredentials.swift" "$src/HomeStore.swift" "$src/MemoryStore.swift" "$src/PluginStore.swift" "$src/ManagementState.swift" "$src/BodyState.swift" "$src/ScreenGeometry.swift" "$src/EngineBridge.swift" "$root"/desktop/macos/Tests/WispBodyTests/*.swift -o "$scratch/NativeChecks"
