...
struct ManagementState {
    ...
    var description: String {
        ...
        case .pets: return "Closed catalog: Wisp orb, Fox, Robot, Bird, Cat, Owl, Sprout, and Capsule. This is the same Wisp. Identity, memory, models, voice, plugins, connections and skills persist. Further official skins toward the eventual collection remain later; no marketplace was queried."
        ...
        case .diagnostics: return "Engine: \(lifecycle.rawValue). Ready means the hidden engine is attached; it does not mean voice is capturing. Settings → Diagnostics shows voice phase, mute, locale, shortcut registration, and categorical microphone and speech permission — not recognized words or assistant replies. Diagnostics include a nonsecret hardware snapshot (CPU, memory kind, GPU/VRAM or unavailable reasons, disk, Ollama names/sizes), qualitative speech headroom, the skill catalog id and demonstration status, mounted plugin/connection/skill counts, the nonsecret current body catalog id, categorical Accessibility TCC, home configured yes or no without a folder path, and last-stop none/clean/forced in process memory. Session telemetry is disabled. No Wisp account is required. Diagnostics never include credentials, keys, bootstrap frames, recognized text, plugin paths, overlay YAML, memory.json, companion UUID, home path, or raw engine errors."
        }
    }
}
