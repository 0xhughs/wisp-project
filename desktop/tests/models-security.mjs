...
async function run(name,withKey=true) {
 ...
 const patch=join(home,'fixture.patch.yml');writeFileSync(patch,readFileSync(new URL('../../spike/profile/wisp.patch.yml',import.meta.url),'utf8').replace('__WISP_ADAPTER__',JSON.stringify(join(runtime,'upstream/wisp-spike/engine/wisp-sdk.ts'))),{mode:0o600});
 const env={...environment(home,meta.pnpm),TSX_TSCONFIG_PATH:join(runtime,'upstream/tsconfig.json'),WISP_SPIKE_LEDGER:join(home,'effects.jsonl'),DEEPSEEK_API_KEY:'unrelated-ambient-canary'};
