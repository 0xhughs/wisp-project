...
import {snapshotFromConfiguration,pluginInventory,PLUGIN_CATALOG_ID} from '../engine/plugin-config.mjs';
...
if(!o['--runtime-root']||!o['--scratch']) { console.error('missing-external-resource: plugin-overlay-runtime.mjs requires --runtime-root and --scratch'); process.exit(2); }
...
const demoPath=join(up,'wisp-product/engine/compatible-plugin.ts');
const developerPath=join(up,'wisp-product/engine/local-permission-plugin.ts');
...
function launch({enabled=false,developer=false,verify=false,extra='',note=''}={}) {
  ...
  const snap=snapshotFromConfiguration({version:1,catalogId:PLUGIN_CATALOG_ID,enabled,config:{note}});
  let patchText=composeOverlay({
    ...
    compatible:enabled?{path:demoPath,config:snap.config,snapshot:snap}:null,
  });
  ...
  const env={...environment(home,meta.pnpm),TSX_TSCONFIG_PATH:join(up,'tsconfig.json'),WISP_COMPANION_ID:memory.companionId,WISP_ENGINE_GENERATION:randomUUID(),WISP_PLUGIN_SNAPSHOT:JSON.stringify(snap),...(enabled?{WISP_COMPATIBLE_LEDGER:join(home,'ledger.compatible.jsonl')}:{}),...(verify?{WISP_COMPATIBLE_VERIFY:'1'}:{}),...(developer?{WISP_PERMISSION_FIXTURES:'1',WISP_PERMISSION_LEDGER:ledger}:{}),WISP_REASONING_LOCAL_KEY:'ollama'};
  ...
  return {client,home,ledger,snap};
}
...
async function initialize(ctx) {
  ...
  const inventory=pluginInventory({snapshot:ctx.snap,tools:notified.tools,transport:notified.transport});
  assert.deepEqual(notified.plugins,inventory.plugins);
  return inventory;
}
...
try {
  ...
  try {
    ...
    assert.ok(inventory.tools.includes('wisp_compatible_check'));
    assert.equal(inventory.plugins[0].id,PLUGIN_CATALOG_ID);
    assert.deepEqual(inventory.plugins[0].tools,['wisp_compatible_check']);
    const compatibleLedger=join(on.home,'ledger.compatible.jsonl');
    const records=()=>existsSync(compatibleLedger)?readFileSync(compatibleLedger,'utf8').trim().split('\n').filter(Boolean):[];
    ...
    for(const decision of [null,'deny','cancel','allow-once']) {
      const running=on.client.request('wisp/verification.compatible',{},120000);running.catch(()=>{});
    ...
    // withhold analogue is cancel-without-allow; expected 0,0,0,1 after mapping null→cancel then deny,cancel,allow.
    assert.deepEqual(counts.slice(1),[0,0,1]);
    assert.equal(counts[0],0);
    results.push({case:'enabled-inventory-and-registry',ok:true,plugins:inventory.plugins,effectCounts:counts,modelCalls0:true});
  ...
  try {
    ...
    assert.equal(inventory.tools.includes('wisp_compatible_check'),false);
    results.push({case:'disable-removes-demo',ok:true});
  ...
  const fixtures=launch({developer:true,enabled:false});
  try {
    const inventory=await initialize(fixtures);
    assert.ok(inventory.tools.includes('wisp_plugin_check'));
    assert.equal(inventory.tools.includes('wisp_compatible_check'),false);
    results.push({case:'developer-fixture-orthogonal',ok:true,tools:inventory.tools});
    await fixtures.client.shutdown();
  } finally { if(!fixtures.client.exited)await fixtures.client.force(); }
