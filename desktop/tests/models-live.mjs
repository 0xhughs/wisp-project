...
const credentialStore=o['--credential-store']||'isolated';
...
child.stdout.on('data',bytes=>{tail+=bytes;const lines=tail.split('\n');tail=lines.pop();for(const line of lines){const f=JSON.parse(line);frames.push(f);appendFileSync(join(run,'private-events.jsonl'),line+'\n',{mode:0o600});if(f.event==='ready')owned.push(f);if(['memory-loaded','memory-error','ready','recall','smoke','connection-test','tested','native-stopped','unavailable','settings-opened','section-selected'].includes(f.event)){const safe={...f};delete safe.recallText;console.log(JSON.stringify(safe));}}});
child.stderr.resume();child.stdin.on('error',()=>{});
const send=op=>{if(!ended)child.stdin.write(JSON.stringify({op})+'\n')};
const input=createInterface({input:process.stdin});input.on('line',op=>{if(['status','smoke','recall','recreate','hide','show','stop'].includes(op))send(op);});
const timer=setInterval(()=>send('status'),3000);
console.log(JSON.stringify({event:'operator-ready',credentialStore,pid:child.pid,run,scratch,commands:['status','smoke','recall','recreate','hide','show','stop']}));
