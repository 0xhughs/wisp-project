...
The catalog lists, and cannot enable: registry/marketplace installs; arbitrary third-party folders; stock executable-tool enablers (`tool-bash`, `tool-fs`, `tool-web`, skill/sub-agent rows, HMR, telemetry, …); MCP/Connection plugins (slice 11); skills (slice 12); web/chat UI plugins; plugin-supplied model or provider replacements. Models remains the sole owner of reasoning keys.
