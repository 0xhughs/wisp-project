...
General's Show Companion reveals and clamps the existing body. It does not activate the body, start audio or replace the engine. The other sections report the current capability boundary: speech, additional bodies, plugin/connection/skill management, grants and proactivity are unavailable. Engine Ready never means voice ready. Models now configures local Ollama and optional protected DeepSeek keys (see `reasoning-models.md`). Memory provides real local folder selection and entry management; see `local-home-memory.md`. Other unimplemented capabilities remain unavailable and no fake inventory or chat surface is supplied.
